...
async function updatePlaylist(songs, id) {
    const[rows] = await pool.query(
        ...
        'UPDATE `playlist` SET `songs` = JSON_REPLACE(`songs`, "$.savedSongs", ?) WHERE id = ?', [songs, id]
    )
}
// get all songs in a playlist
async function getSongsInPlaylist(id) {
    const [rows] = await pool.query(
        //'SELECT songs->>"$.savedSongs" AS saved_songs FROM `playlist` WHERE id = ?', [id]
        'SELECT songs FROM `playlist` WHERE id = ?', [id]
    )
    return rows[0].songs.savedSongs
    // return rows[0].saved_songs
}
